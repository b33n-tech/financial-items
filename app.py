# Streamlit prototype: Items Budgeting App (fixed)
...
def add_default_categories():
    ...
    for name, ip, uv, ul in defaults:
        try:
            cur.execute(
                "INSERT INTO categories (name, items_per_period, unit_value, unit_label) VALUES (?, ?, ?, ?)",
                (name, ip, uv, ul),
            )
...
def list_categories():
    ...
    cur = conn.cursor()
    cur.execute(
        "SELECT id, name, items_per_period, unit_value, unit_label FROM categories ORDER BY id"
    )
    rows = cur.fetchall()
    conn.close()
    return rows
...
def record_click(category_id, reason, amount):
    ...
    cur = conn.cursor()
    cur.execute(
        "INSERT INTO clicks (category_id, ts, reason, amount) VALUES (?, ?, ?, ?)",
...
def get_clicks_dataframe():
    ...
    cur = conn.cursor()
    cur.execute(
        "SELECT c.id, cat.name, c.ts, c.reason, c.amount FROM clicks c JOIN categories cat ON c.category_id = cat.id ORDER BY c.id DESC"
    )
    rows = cur.fetchall()
    conn.close()
    if rows:
        return pd.DataFrame(rows, columns=["id", "category", "ts", "reason", "amount"])
    return pd.DataFrame(columns=["id", "category", "ts", "reason", "amount"])
...
def clicks_this_period(category_id, year=None, month=None):
    ...
    cur.execute(
        "SELECT COUNT(*) FROM clicks WHERE category_id = ? AND ts >= ? AND ts < ?",
        (category_id, start, next_month),
    )
    n = cur.fetchone()[0]
    conn.close()
    return n
...
with st.sidebar:
    ...
    with st.form("new_cat", clear_on_submit=True):
        ...
        unit_value = st.number_input("Valeur unitaire (€)", min_value=0.0, value=10.0)
        unit_label = st.text_input("Libellé unité (ex: burger)", value="item")
        if st.form_submit_button("Créer") and name.strip():
            create_category(name.strip(), items_per_period, unit_value, unit_label.strip())
    ...
    if st.button("Réinitialiser la DB (supprime tout)"):
        confirm_reset = st.checkbox(
            "Je confirme la suppression totale (cocher pour activer)"
        )
        if confirm_reset:
